...
- **侧边栏**：`List(selection:)` + `.listStyle(.sidebar)`，顶上一个搜索框。
  ...
  行样式对齐 Raycast Preferences：`SettingsTileIcon` 18pt 色块、色块与标题约 10pt 间距、
  Regular 13pt 标题、选中用中性浅灰而非系统强调蓝。
...
四条规则，主搜索列表（`ResultListView`）与插件里的列表都必须遵守。
...
- **跟随滚动不做动画。** `scrollTo` 不要包 `withAnimation`：长按方向键会连发按键，
  每次按键都起一段动画，列表就一直在追赶按键，动画被反复打断 —— 体感是卡顿，不是顺滑。
  跨过边缘时瞬时跳一档，静止的部分保持静止。`Duration` 里因此**没有**「滚动定位」这个时长。
- **只有键盘才触发滚动跟随，悬停不触发。** 主搜索列表里悬停也会改选中项（`PaletteSelection`
  的 `hoverArmed`），但列表要听 `keyboardScrollToken`、不能听 `selectedIndex`：指针底下
  那一行本来就在可视区里，跟着 `selectedIndex` 滚会让光标扫过**第一行**时命中
  `scrollTo(anchor: .top)`，把内容往上顶一个内边距 —— 表现就是「跳一下」。
...
| 侧边栏图标 | `SettingsTileIcon`（18pt 彩色色块 + Medium 白色 SF Symbol） | 设置窗口侧栏；对齐 Raycast Preferences 的瓷砖图标 |
...
- **动画时长的用途要对**：0.18 用于面板出现、0.12 用于悬停，不要混用，也不要把
  列表的选中跟随做成动画（见 §2b「列表的滚动跟随与悬停高亮」）。
