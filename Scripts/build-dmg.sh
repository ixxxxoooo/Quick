...
#
# 与 `restart-dev.sh` 的分工：那条跑 Debug（开发渠道，Quick Dev.app），
# 这条跑 Release（正式渠道，Quick.app，通用二进制），产物落在 dist/。
#
# 用法：bash Scripts/build-dmg.sh
#
# 输出：dist/Quick-<版本>.dmg，里面是
#   Quick.app                   主程序
...
DIST="dist"
REPO_URL="github.com/ixxxxoooo/Quick"
...
#   · 证书「Quick」签名 → 要求是 `identifier + certificate leaf`，重装 / 升级都认同一份授权；
#   · ad-hoc 签名 → 要求退化成 `cdhash`（就是「这一个二进制」的指纹），
#     **每构建一次就换一个身份**，用户每次升级都得重新授权一遍——而且这时把 App
#     重拖进系统设置也没用，那一行早就在列表里了。
# 所以 ad-hoc 只能显式降级：设 QUICK_ALLOW_ADHOC=1（默认拒绝，见 2/6 的身份守门）。
#
...
if security find-identity -p codesigning 2>/dev/null | grep '"Quick"' >/dev/null; then
    ...
    echo "找不到自签名证书「Quick」，拒绝产出 ad-hoc 签名的 DMG：" >&2
    echo "  该签名的身份会随每次构建变化，用户升级一次就要重新授权一次屏幕录制。" >&2
    echo "  · CI：把证书导出成 .p12 配进仓库 secrets（见 AGENTS.md「发布」一节）" >&2
    echo "  · 本机：钥匙串登录项里应有「Quick」证书（security find-identity -p codesigning）" >&2
    echo "  · 只想验打包流程：QUICK_ALLOW_ADHOC=1 bash Scripts/build-dmg.sh" >&2
    exit 1
fi
LOG="$(mktemp -t quick-release)"
# $SIGN_OVERRIDES 故意不加引号：要把两条 build setting 拆成两个参数传给 xcodebuild。
# shellcheck disable=SC2086
if ! xcodebuild -project Quick.xcodeproj -scheme Quick -configuration "$CONFIG" \
...
# 可用 QUICK_IMAGE_TOOL=diskutil|hdiutil 强制，便于在两种系统上验证同一条路径。
IMAGE_TOOL="${QUICK_IMAGE_TOOL:-}"
