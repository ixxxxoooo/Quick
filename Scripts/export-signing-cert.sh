#!/usr/bin/env bash
#
# 把本机已有的「Quick」代码签名身份导出成 CI 用的 .p12。
#
...
#
# 产出（默认 ~/.config/quick/，不进 git）：
#   quick-signing.p12 / .password / .base64
#
# @author ygw
set -euo pipefail
...
# 登录串里可能还有别的自签名身份；下一步用临时钥匙串滤到只剩 Quick。
...
security export -k "$FILTER_KC" -t identities -f pkcs12 \
    -o "$OUT_DIR/quick-signing.p12" -P "$P12_PASSWORD"
...
security import "$OUT_DIR/quick-signing.p12" -k "$VERIFY_KC" -P "$P12_PASSWORD" >/dev/null
echo "    复验内容："
...
if [ "$UPLOAD" = "1" ]; then
    ...
    echo "    已写入 QUICK_CERT_P12_BASE64 / QUICK_CERT_P12_PASSWORD"
    gh secret list
fi
