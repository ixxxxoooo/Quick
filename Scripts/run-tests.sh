...
# 前置检查：插件的触发词必须全局唯一
#
# 触发词是字符串空间里的隐式命名空间，编译器看不见 —— 两个插件声明同一个词，
# 同一条查询会同时唤醒它们。由 Scripts/check-trigger-words.py 守着。
# ---------------------------------------------------------------------------
if [[ -x Scripts/check-trigger-words.py ]]; then
    if ! Scripts/check-trigger-words.py; then
        exit 1
    fi
fi
