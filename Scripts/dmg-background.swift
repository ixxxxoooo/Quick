...
//       --name Quick --version 0.0.2 --repo github.com/ixxxxoooo/quick
//
// @author ygw
import AppKit
...
func text(
    _ string: String, _ f: NSFont, _ color: NSColor, kern: CGFloat = 0,
    baseline: CGFloat = 0
) -> NSAttributedString {
...
func draw(_ string: NSAttributedString, centerX: CGFloat, topY: CGFloat) {
    ...
    string.draw(
        in: NSRect(
            x: snap(centerX - size.width / 2), y: snap(topY),
...
func draw(_ string: NSAttributedString, left: CGFloat, centerY: CGFloat) {
    ...
    string.draw(
        in: NSRect(
            x: snap(left), y: snap(centerY - size.height / 2),
            width: size.width, height: size.height))
}
...
func renderImage(_ config: Config, scale: CGFloat) -> NSBitmapImageRep? {
    ...
    if let gradient = CGGradient(
        ...
        colors: [
            ...
            NSColor(srgbRed: 0.910, green: 0.925, blue: 0.957, alpha: 1).cgColor
        ] as CFArray,
        locations: [0, 0.52, 1])
    {
    ...
    if let glow = CGGradient(
        colorsSpace: space,
        colors: [
            brand.withAlphaComponent(0.15).cgColor,
            brand.withAlphaComponent(0).cgColor
        ] as CFArray,
        locations: [0, 1])
    {
        ...
        cg.drawRadialGradient(
            CGGradient(
                colorsSpace: space,
                colors: [
                    NSColor(srgbRed: 1, green: 1, blue: 1, alpha: 0.85).cgColor,
    ...
    let centerX = width / 2
    let kicker = NSBezierPath(
        roundedRect: NSRect(x: centerX - 11, y: 34, width: 22, height: 3),
    ...
    if !config.version.isEmpty {
        wordmark = NSAttributedString(
            string: "\(config.appName) \(config.version)",
            attributes: [
    ...
    draw(
        text(
            "拖拽到「应用程序」即可安装　·　Drag to Applications to install",
    ...
    if !config.repo.isEmpty {
        ...
        footer.draw(
            in: NSRect(
                x: snap(width - 30 - size.width), y: snap(height - 30),
                width: size.width, height: size.height))
    }
