...
struct RecentPromotionTests {
    ...
    func promotesRecentsInOrder() {
        ...
        // c 最近用过、然后是 a
        let promoted = PaletteCoordinator.promotingRecents(items, recents: ["c", "a"])
    ...
    func keepsRemainingOrder() {
        ...
        let promoted = PaletteCoordinator.promotingRecents(items, recents: ["c"])
    ...
    func noRecentsIsNoop() {
        ...
        #expect(PaletteCoordinator.promotingRecents(items, recents: []).map(\.id) == ["a", "b"])
    }
    ...
    func ignoresUnknownRecentIDs() {
        ...
        let items = [item("a"), item("b")]
        let promoted = PaletteCoordinator.promotingRecents(items, recents: ["gone", "b"])
    ...
    func preservesCount() {
        ...
        let promoted = PaletteCoordinator.promotingRecents(items, recents: ["item-7", "item-19", "item-3"])
