...
/// 文件搜索的偏好
///
/// 纯数据：会话按它构造谓词、过滤与截断，至于值从哪个键读出来、用户什么时候改的，
/// 由 `FileSearchPreferences.current` 负责。
struct FileSearchSettings: Sendable, Equatable {
...
/// 文件搜索偏好的读取与映射
///
/// 读键的动作发生在**每次搜索时**，不是在会话 init 里读一次：
/// 设置页可以在应用运行期间随时改，捕获一次的话用户就会看到「改了没反应」。
enum FileSearchPreferences {

    /// 设置页里可选的结果条数
    static let resultChoices = [20, 50, 100, 200]

    /// 设置页在「从未设置过」时的默认条数
    static let defaultMaxResults = 50

    /// 把存储值映射成合法的结果上限
    ///
    /// `UserDefaults.integer(forKey:)` 对没写过的键返回 0，手改过的值也可能是任意整数，
    /// 所以非法输入一律回落到设置页显示的默认档 —— 上限为 0 会让搜索结果永远是空的。
    static func maxResults(storedValue: Int) -> Int {
        resultChoices.contains(storedValue) ? storedValue : defaultMaxResults
    }

    /// 当前的三项设置
    static func current(defaults: UserDefaults = .standard) -> FileSearchSettings {
        FileSearchSettings(
            maxResults: maxResults(
                storedValue: defaults.integer(forKey: PluginSettingKey.FileSearch.maxResults)),
            // 没写过这两个键时要按设置页显示的档位处理：忽略隐藏文件是「开」，搜索内容是「关」
            ignoreHidden: boolValue(
                forKey: PluginSettingKey.FileSearch.ignoreHidden,
                fallback: true,
                defaults: defaults
            ),
            includeContents: boolValue(
                forKey: PluginSettingKey.FileSearch.includeContents,
                fallback: false,
                defaults: defaults
            )
        )
...
/// 会话只负责把 `NSMetadataItem` 读成结果，再交给这里。
enum FileSearchFiltering {
