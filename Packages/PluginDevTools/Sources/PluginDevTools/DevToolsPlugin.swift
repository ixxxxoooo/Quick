...
/// 开发者工具集插件
///
/// 集合 12 个轻量开发工具为一个 Plugin，
/// 内部按子功能文件夹隔离，对外呈现为统一入口。
@MainActor
public final class DevToolsPlugin: QuickPlugin {
    ...
    private let log = QuickLog.plugin(DevToolsPlugin.id)
    ...
    public func makeView() -> AnyView {
        AnyView(DevToolsRootView(plugin: self, tools: tools))
    }

    public func activate() {
        log.notice("插件已激活，已注册 \(self.tools.count, privacy: .public) 个子工具")
    }
...
struct DevToolsRootView: View {
    ...
    var body: some View {
        Group {
            if let id = selectedID, let tool = plugin.tool(for: id) {
                // 直接显示子工具的完整面板
                VStack(spacing: 0) {
                    // 子工具标题栏（有返回按钮可回到工具列表）
        ...
        .onAppear {
            if selectedID == nil, let toolID = plugin.selectedToolID {
                selectedID = toolID
            }
        }
    }
    ...
    private var toolGrid: some View {
        ScrollView {
            LazyVGrid(
                ...
                ForEach(plugin.toolEntries) { entry in
                    Button {
                        selectedID = entry.id
                    } label: {
